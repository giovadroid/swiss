    ...
    for (shell_name, target) in &config.shells {
        ...
        match mode {
            ...
            ShellMode::ManagedLoader => {
                ...
                steps.extend(nushell_managed_loader_steps(config, target, context)?);
            }
            ShellMode::Snippet | ShellMode::Profile => {
                let target_file = target
                    ...
                    .ok_or_else(|| {
                        anyhow::anyhow!("Shell '{}' needs an explicit 'target'", shell_name)
                    })?;
                for (module_name, spec) in resolve_modules(config, &target.modules, shell_name)? {
                    if let Some(content) = render_module(kind, spec) {
                        steps.push(Step::PatchBlock {
                            target: target_file.clone(),
                            block: module_name.clone(),
                            content,
                        });
                    }
                }
...
fn nushell_managed_loader_steps(
    config: &SwissConfig,
...
/// `swiss shell print`: emits all blocks for one shell to stdout.
pub fn render_print(config: &SwissConfig, kind: ShellKind) -> Result<String> {
    let Some(target) = config.shells.get(kind.name()) else {
        bail!("Shell '{}' is not configured in the manifest", kind.name());
    };

    let module_names: Vec<String> = if kind == ShellKind::Nushell {
        target
            .env_modules
            .iter()
            .chain(target.conf_modules.iter())
            .cloned()
            .collect()
    } else {
    ...
    for (module_name, spec) in resolve_modules(config, &module_names, kind.name())? {
        if let Some(content) = render_module(kind, spec) {
            output.push_str(&render_block(module_name, &content));
            output.push('\n');
        }
    }
    Ok(output)
...
mod tests {
    ...
    #[test]
    fn shell_steps_skip_disabled_shells() {
        let config = shell_config();
        ...
        // zsh produces blocks against its default target
        assert!(steps.iter().any(|step| matches!(
            step,
            Step::PatchBlock { target, block, .. } if target == "~/.zshrc" && block == "starship"
        )));
    ...
    #[test]
    fn print_renders_bounded_blocks() {
        let config = shell_config();
        let output = render_print(&config, ShellKind::Zsh).unwrap();
